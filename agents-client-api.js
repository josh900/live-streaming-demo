...
let isPushToTalkActive = false;let reconnectAttempts = 10;
...
function setupPushToTalkSession() {
  // Setup recording session without starting transcription
  // This may involve initializing the audio context and creating the Deepgram connection
  // but not sending audio data yet
}
function cleanupPushToTalkSession() {
  // Clean up any resources used for Push to Talk mode
}
...
function handleTranscription(data) {
  if (!isRecording || (isPushToTalkEnabled && !isPushToTalkActive)) return;
  const transcript = data.channel.alternatives[0].transcript;
  if (data.is_final || isPushToTalkEnabled) {
