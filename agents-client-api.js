...
async function continueStreaming(content) {
  try {
    logger.debug('Continuing streaming with content:', content);

    if (!persistentStreamId || !persistentSessionId) {
      logger.error('Persistent stream not initialized. Cannot continue streaming.');
      return;
    }
    ...
    const playResponseData = await playResponse.json();
    logger.debug('Streaming continuation response:', playResponseData);
    ...
    logger.error('Error during streaming continuation:', error);
  }
}
...
async function sendChatToGroq() {
  ...
  try {
    ...
    let pauseInterval = setInterval(async () => {
      if (!done) {
        await continueStreaming('<break time="1s"/>');
      } else {
        ...
        for (const line of lines) {
          if (line.startsWith('data:')) {
            ...
            try {
              ...
              // Continue streaming with the new content
              if (content.trim() !== '') {
                await continueStreaming(content);
              }
            } catch (error) {
