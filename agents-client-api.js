...
function togglePushToTalk() {
  ...
  pushToTalkButton.style.display = isPushToTalkMode ? 'block' : 'none';
  if (isPushToTalkMode) {
    setupPushToTalkConnection();
  }
}
function setupPushToTalkConnection() {
  // Setup Deepgram connection without endpointing and utterances
  // This should be similar to the existing Deepgram setup, but with modified options
}
...
function toggleAutoSpeak() {
  if (isPushToTalkMode) {
    showErrorMessage('Cannot enable Auto-Speak while Push to Talk is active');
    return;
  }
