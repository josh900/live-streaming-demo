...
function handleTranscription(data) {
  ...
  if (data.is_final) {
    ...
    if (!interimMessageAdded) {
      msgHistory.innerHTML += `<span data-interim style="opacity:0.5"><u>Userabc (Interim):</u> ${interimTranscript}</span><br>`;
      interimMessageAdded = true;
    } else {
      const interimElement = msgHistory.querySelector('span[data-interim]');
      if (interimElement) {
        interimElement.innerHTML = `<u>Userxyz (Interim):</u> ${interimTranscript}`;
      }
    }
