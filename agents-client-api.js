...
function updateButtonText(text) {
  const logoWrapper = document.getElementById('logo-wrapper');
  const svgDoc = logoWrapper.querySelector('svg');
  if (svgDoc) {
    const textElement = svgDoc.getElementById('button-text');
    if (textElement) {
      textElement.textContent = text;
      // Adjust text position and size based on the new content
      const bbox = textElement.getBBox();
      const svgWidth = svgDoc.viewBox.baseVal.width;
      const svgHeight = svgDoc.viewBox.baseVal.height;
      textElement.setAttribute('x', (svgWidth - bbox.width) / 2);
      textElement.setAttribute('y', svgHeight / 2 + bbox.height / 4);
      textElement.setAttribute('font-size', Math.min(70, 1000 / text.length));
    }
  }
}
...
export async function initialize() {
  ...
  <svg version="1.1" viewBox="0 0 710 300" xmlns="http://www.w3.org/2000/svg">
    <defs>
    ...
    <text id="button-text" x="250" y="177" font-family="Arial, sans-serif" font-size="70" font-weight="bold" fill="#2E3E3D">Hold to Talk</text>
  </svg>
`;
