...
connectButton.onclick = async () => {
  ...
  try {
    ...
    if (!sdpResponse.ok) {
      throw new Error(`HTTP error ${sdpResponse.status}`);
    }
...
async function startStreaming(assistantReply) {
  try {
    ...
    if (!playResponse.ok) {
      ...
      console.error('Error response from D-ID API:', errorText);
      throw new Error(`HTTP error ${playResponse.status}: ${errorText}`);
    }

    const playResult = await playResponse.json();
    console.log('Streaming started:', playResult);

    // Add a check here to see if the video is actually playing
    setTimeout(() => {
      if (!videoIsPlaying) {
        console.warn('Video is not playing after 5 seconds. Current video element state:', videoElement.readyState);
      }
    }, 5000);
...
function logState() {
  ...
  console.log('Peer Connection State:', peerConnection ? peerConnection.connectionState : 'No peer connection');
  console.log('ICE Connection State:', peerConnection ? peerConnection.iceConnectionState : 'No peer connection');
